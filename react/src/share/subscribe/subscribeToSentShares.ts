...
/**
 * Establishes a realtime subscription to shares sent by the current user.
 * Subscribes to the `share_public` table filtered by sender_user_id.
 ...
 */
export default function subscribeToSentShares(
	get: () => ShareSlice,
	...
	return Effect.gen(function* subscribeGen($) {
		...
		const cleanup = createRealtimeSubscription({
			...
			onEvent: (payload: unknown) => 
				handleShareSubscribeEvent(payload, supabaseClient, { get, shareType: 'sent' }),
		});
