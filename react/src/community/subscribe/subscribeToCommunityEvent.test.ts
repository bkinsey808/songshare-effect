...
import subscribeToCommunityEvent from "./subscribeToCommunityEvent";
...
vi.mock("./handleCommunityEventSubscribeEvent");

describe("subscribeToCommunityEvent", () => {
	...
	it("fails when no supabase client is available", async () => {
		...
		const eff = subscribeToCommunityEvent("c1", () =>
	...
	it("creates a subscription, wires events, and returns a cleanup that calls underlying cleanup", async () => {
		...
		vi.mocked(createRealtimeSubscription).mockImplementation(
			({ client, tableName, filter, onEvent }) => {
				...
				expect(tableName).toBe("community_event");
		...
		const eff = subscribeToCommunityEvent("c1", get);
