...
import addEventToCommunity from "./addEventToCommunity";

vi.mock("@/shared/fetch/postJson");

describe("addEventToCommunity", () => {
	// Per-test setup: avoid lifecycle hooks to satisfy lint rules and keep tests explicit
	const FIRST_CALL = 1;
	...
	it("sets loading and clears error on success", async () => {
		...
		const eff = addEventToCommunity("c1", "e1", get);
