...
export default function addActivePublicSongIds(
	...
	return (songIds: readonly string[]): Effect.Effect<void, Error> => {
		...
		function processSong(song: unknown, out: Record<string, SongPublic>): void {
				...
				const normalizedSlides: Record<string, { slide_name: string; field_data: Record<string, string> }> = {};
				for (const key of Object.keys(rawSlides)) {
					...
					for (const field of allRequiredFields) {
						field_data[field] =
							typeof rawFieldData[field] === "string" ? rawFieldData[field] : "";
					}
					...
					if (songName.length < MIN_SONG_NAME_LENGTH || songName.length > MAX_SONG_NAME_LENGTH || /\s{2}/.test(songName)) {
						songName = songName.length > MIN_STRING_LENGTH && songName.length <= MAX_SONG_NAME_LENGTH ? songName.trim().replaceAll(/\s{2,}/g, " ") : "Untitled Song";
					}
						// Extract and validate song_slug (must match slug format)
						let songSlug = typeof song["song_slug"] === "string" ? song["song_slug"] : "";
						if (!/^[a-z0-9-]+$/.test(songSlug) || songSlug.startsWith("-") || songSlug.endsWith("-") || songSlug.includes("--")) {
							// Generate a valid slug from song_name if slug is invalid
							songSlug = songName.toLowerCase().replaceAll(/[^a-z0-9]+/g, "-").replaceAll(/^-+|-+$/g, "").replaceAll(/-{2,}/g, "-") || "untitled";
						}
						...
						const createdAt = typeof song["created_at"] === "string" ? song["created_at"] : new Date().toISOString();
						const updatedAt = typeof song["updated_at"] === "string" ? song["updated_at"] : new Date().toISOString();
					...
					console.warn(`[addActivePublicSongIds] Normalized song decode failed, trying absolutely minimal song for ${id}`);
					const absolutelyMinimalSong: {
						...
						if (minimalDecodeResult._tag === "Right") {
							console.warn(`[addActivePublicSongIds] Created absolutely minimal song for ${id} with basic fields`);
							out[id] = minimalDecodeResult.right;
							return;
						}
						console.warn(`[addActivePublicSongIds] Even absolutely minimal song decode failed for ${id}:`, minimalDecodeResult.left);
					} catch (error) {
