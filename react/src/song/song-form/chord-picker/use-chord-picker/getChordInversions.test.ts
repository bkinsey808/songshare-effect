...
describe("getChordInversions", () => {
	it("returns an empty array for an unknown shape code", () => {
		// Act
		const result = getChordInversions(C_MAJOR_ROOT, UNKNOWN_SHAPE_CODE);
	...
	it("returns the correct inversion count, numbers, and bass notes for C major", () => {
		// Act
		const result = getChordInversions(C_MAJOR_ROOT, MAJOR_SHAPE_CODE);
	...
	it("sets matchedShape when the re-rooted spelling matches a known chord (augmented chord)", () => {
		...
		// Act
		const result = getChordInversions(C_MAJOR_ROOT, AUG_SHAPE_CODE);
