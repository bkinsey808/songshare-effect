...
export function useGridDragAndDrop({
	...
	// Use slideIds directly as sortable items for grid
	...
	const handleDragEnd = (event: DragEndEvent): void => {
		...
		if (over && active.id !== over.id) {
			const activeIndex = slideIds.indexOf(active.id as string);
			const overIndex = slideIds.indexOf(over.id as string);

			if (activeIndex !== -1 && overIndex !== -1) {
				const newOrder = arrayMove(slideIds, activeIndex, overIndex);
