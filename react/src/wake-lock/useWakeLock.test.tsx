...
import { getWakeLockSentinel, setWakeLockSentinel } from "@/react/wake-lock/sentinel";
...
describe("useWakeLock", () => {
	...
	it("requests wake lock on mount when supported", async () => {
		...
		const requestModule = await import("@/react/wake-lock/requestWakeLock");
	...
	it("toggleWakeLock calls release when active and request when inactive", async () => {
		...
		const requestModule = await import("@/react/wake-lock/requestWakeLock");
		const releaseModule = await import("@/react/wake-lock/releaseWakeLock");
		const reqSpy = vi.spyOn(requestModule, "default").mockResolvedValue(true);
