...
/**
 * Establishes a realtime subscription to the `tag_library` table. Returns
 ...
 *
 * @param get - Getter for the `TagLibrarySlice` used by event handlers.
 * @returns An Effect that resolves to a `() => void` cleanup function.
 */
export default function subscribeToTagLibraryEffect(
	get: () => TagLibrarySlice,
	...
	return Effect.gen(function* subscribeGen($) {
		...
		const cleanup = createRealtimeSubscription({
			...
			onEvent: (payload: unknown) => handleTagLibrarySubscribeEvent(payload, get),
		});
