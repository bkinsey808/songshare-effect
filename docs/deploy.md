...
Before deploying any changes to production, follow these practices to ensure reliability and prevent outages:

### Pre-Deploy Validation

Always validate the build locally before deploying:
...
# Build everything to catch build-time errors
npm run build:all
...
- For most changes: `npm run deploy`
  - Builds and deploys both frontend (Pages) and API (Worker)
  - Does **not** purge cache

- For user-facing changes: `npm run deploy:full`
  - Builds, deploys, **and** purges cache
  - Use this when updating UI, API behavior, or fixing bugs
  - Ensures users immediately see new code instead of cached versions
  - Without cache purge, users may see old versions for 5-10 minutes

**When to use `deploy:full`:**
...
After verifying, watch the API logs for any errors:
...
# - Unexpected errors in user requests
```

### Never Deploy Broken Code

```bash
# ❌ WRONG - Skipping checks
npm run deploy

# ✅ RIGHT - Always validate first
npm run lint && npm run test:unit && npm run build:all && npm run deploy
```
...
- `api/wrangler.toml` and `api/src/server.ts` - worker code
