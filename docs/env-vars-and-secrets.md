...
Lists the subset of vars that are pushed to **Cloudflare Workers** as secrets. This is the authoritative list for `wrangler secret put`. Contains only vars the Worker runtime actually reads — no build-time-only VITE_ vars, no infra-only vars.
...
```
Note: helpers automatically prepend `VITE_` — pass the name *without* the prefix.
