...
Clients must extract `data` before validating or using the payload. Passing the full response object to a validator that expects the domain object will fail because fields like `song_id` live inside `data`, not at the top level.
...
### Supabase mutation errors do not reject automatically

Supabase/Postgrest may resolve a mutation with `{ data: null, error }` for foreign key violations and other constraint failures. Do not assume `Effect.tryPromise` or `catch` logic will see these errors automatically. Always check `result.error` after mutations.

### Known repair paths can retry safely

When a failure mode is understood, it is reasonable to add targeted repair logic that fixes the underlying issue and retries the operation.

Example:

- If a `song_library` insert fails with `song_library_song_id_fkey`, the missing row is in `song`.
- Because `song_public` has a foreign key to `song`, an existing `song_public` row implies the `song` row already exists.
- In that case, use request data such as `song_owner_id` to create the missing `song` row, then retry the library insert.

### Successful API responses can still fail in the UI

A `200` response only confirms the server succeeded. The client can still fail to parse, validate, or commit the response into state. When debugging end-to-end issues, trace the full path:
