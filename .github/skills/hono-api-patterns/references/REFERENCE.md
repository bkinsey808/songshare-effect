...
app.onError((error, c: Context) => {
	...
	if (error instanceof NotFoundError) {
		return c.json({ error: `${error.resource} not found`, id: error.id }, 404);
	}
 ...
 */
function executeEffect<A, E, R>(effect: Effect.Effect<A, E, R>, c: Context): Promise<Response> {
	return Effect.runPromise(effect).then(
		...
		(error) => {
			...
			const [status, message] = errorMap[error.constructor.name] || [500, "Internal server error"];
 ...
 */
export function withDependencies<T extends Record<string, any>>(dependencies: T) {
