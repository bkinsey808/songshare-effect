...
if (!profileDir) {
	console.error(
		"Error: PROFILE_DIR not provided. Either set env PROFILE_DIR or pass as first arg.",
	);
...
(async () => {
	...
	try {
		...
		if (!stat.isDirectory()) {
			console.warn(
				"Warning: PROFILE_DIR exists but is not a directory. Proceeding anyway.",
			);
		}
	} catch {
		console.warn(
			"Warning: PROFILE_DIR does not exist yet. Playwright will create it when launching Chrome.",
		);
	}
	...
	console.log(
		"If you prefer to sign in manually first in Windows Chrome, do that now using the same profile directory, then close Chrome before continuing.",
	);
	...
	try {
		...
		if (envChrome && fs.existsSync(envChrome)) {
			...
			console.log(
				"Falling back to Playwright channel: chrome (this requires `npx playwright install chrome`)",
			);
	...
	console.log(
		"Press ENTER here after you have completed sign-in and closed any consent dialogs.",
	);
	...
	console.log(
		"Done. Use tests/storageState.json in your Playwright tests (test.use or playwright.config).",
	);
	process.exit(0);
})();
