...
if (!wsUrl) {
	console.error(
		"Usage: node capture-cdp.js <webSocketDebuggerUrl> [durationMs] [--json]",
	);
	process.exit(1);
}
...
ws.on("open", () => {
	...
	console.log(
		"📡 Discovering and attaching to page targets. Listening for Console and Network events for",
		durationMs,
		"ms",
	);
	...
	if (!noTimeout) {
		setTimeout(() => {
			...
			consoles.forEach((c, i) =>
				console.log(`   ${i + 1}. [${c.level}] ${c.text}`),
			);
			console.log("\n  • Network requests captured:", responses.length);
			responses.forEach((r, i) =>
				console.log(
					`   ${i + 1}. [${r.method}] ${r.url} -> ${r.status || "N/A"}`,
				),
			);
			ws.close();
			process.exit(0);
		...
		console.log(
			"⏳ No-timeout mode enabled — the capture will run until you stop it (SIGINT/SIGTERM).",
		);
	}
});
...
ws.on("message", (data) => {
	try {
		...
		if (msg.method === "Target.attachedToTarget" && msg.params) {
			...
			if (sId) {
				sessions.set(sId, {
					targetId: msg.params.targetInfo && msg.params.targetInfo.targetId,
				});
				enableDomains(sId);
			}
		}
		...
		function handleNetworkEvent(m, p) {
			...
			if (m === "Network.responseReceived") {
				...
				console.log(
					`⬇️  Response: ${entry.method} ${entry.url} -> ${entry.status} (${entry.mimeType})`,
				);
			}
