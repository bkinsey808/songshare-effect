...
async function attachViaBrowserWebSocket() {
	// Try to read DevTools url from the secure chrome debug log
	...
	ws.on("message", (data) => {
		...
		// Responses to our outgoing calls
		if (msg.id && pending.has(msg.id)) {
			...
			if (entry.type === "createTarget") {
				...
				// store sessionId for future
				ws.__sessionId = sessionId;
			}
			return;
		}

		// Events from browser may include Target.receivedMessageFromTarget which contains inner message
		if (
		...
		// Also handle top-level events (rare for console)
		if (msg.method) {
...
async function openTargetAndAttach() {
	try {
		...
		// Fallback: attach via browser-level websocket by reading the debug log
