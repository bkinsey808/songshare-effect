...
# Get the first tab
TAB_DATA=$(curl -s "http://127.0.0.1:$CHROME_DEBUG_PORT/json" | head -1)
TAB_ID=$(echo "$TAB_DATA" | grep -o '"id":"[^"]*"' | cut -d'"' -f4)
WS_URL=$(echo "$TAB_DATA" | grep -o '"webSocketDebuggerUrl":"[^"]*"' | cut -d'"' -f4)
...
echo -e "${GREEN}✓ Found tab: $TAB_ID${NC}"
...
if ! command -v node &> /dev/null; then
    ...
    # Run the Node.js console log reader
