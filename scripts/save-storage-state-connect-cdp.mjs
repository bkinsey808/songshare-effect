...
const REMOTE_DEBUGGING_URL = process.env.REMOTE_DEBUGGING_URL || 'http://127.0.0.1:9222';
const OUTPUT = process.env.OUTPUT || path.resolve(process.cwd(), 'tests', 'storageState.json');
...
async function main() {
  console.log(`Connecting to remote debugging endpoint: ${REMOTE_DEBUGGING_URL}`);

  let browser;
  try {
    // Attempt to connect over CDP to the running Windows Chrome
    browser = await chromium.connectOverCDP(REMOTE_DEBUGGING_URL, { timeout: 15000 });
  } catch (err) {
    console.error('Failed to connect to Chrome over CDP:', err.message || err);
    console.error('Make sure Chrome is running on Windows with: --remote-debugging-port=9222 and that you can reach http://127.0.0.1:9222 from WSL.');
    process.exitCode = 2;
    return;
  }
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      ...
      console.log('No existing contexts found; creating a temporary context and navigating to the app to ensure cookies are loaded.');
      ...
      try {
        ...
        console.warn(`Warning: navigation to ${BASE_URL} failed:`, err.message || err);
