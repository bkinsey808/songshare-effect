 ...
 */
const isMockInternalVarAccess = (
    node: TSESTree.Node,
    mockInternalVars: Set<string>,
): boolean => {
...
const rule: RuleModule<MessageIds> = {
    ...
    create(context) {
        ...
        const trackViMockVar = (
            id: TSESTree.Node,
            init: TSESTree.Node | null | undefined,
        ): void => {
            if (
                id.type === 'Identifier' &&
                isViMockExpression(init, viMockVars)
            ) {
        ...
        const trackMockInternalVar = (
            ...
            if (
                stripped == null ||
                !isMockInternalAccess(stripped, viMockVars)
            ) {
                return;
            }
            ...
            if (
                getMockInternalProperty(stripped, viMockVars) === 'calls.length'
            ) {
                return;
            }
        ...
        return {
            ...
            CallExpression(node: TSESTree.CallExpression): void {
                ...
                if (
                    mockProp !== null ||
                    isMockInternalVarAccess(arg, mockInternalVars)
                ) {
                    ...
                    const propMessageId: Record<
                        MockInternalProperty,
                        MessageIds
                    > = {
