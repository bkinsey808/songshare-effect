...
/**
 * Verify a user session JWT using the environment's `SUPABASE_JWT_SECRET`.
 *
 * This helper looks up the `SUPABASE_JWT_SECRET` from the provided environment-like
 ...
 * @param envLike - Environment-like object used to resolve `SUPABASE_JWT_SECRET`.
...
export default function verifyUserSessionToken(
	...
	return Effect.tryPromise({
		try: () => {
			const jwtSecret = getEnvString(envLike, "SUPABASE_JWT_SECRET");
