	...
	return Effect.gen(function* communityShareRequestCreateGen($) {
			...
			Effect.tryPromise({
				try: () =>
					supabase
						...
						.single(),
				catch: (err) => new DatabaseError({ message: extractErrorMessage(err, "Failed to verify community membership") }),
			}),
		);
		if (membership.error || membership.data?.status !== "joined") {
			return yield* $(Effect.fail(new ValidationError({ message: "Only joined community members can submit share requests" })));
		}
		const insertResult = yield* $(
			Effect.tryPromise({
				...
				catch: (err) => new DatabaseError({ message: extractErrorMessage(err, "Failed to submit community share request") }),
			}),
		);
		if (insertResult.error) {
			return yield* $(Effect.fail(new DatabaseError({ message: insertResult.error.message ?? "Failed to submit community share request" })));
		}
		return { success: true };
	});
}
