	...
	return Effect.tryPromise({
		try: async () => {
			const { data, error } = await client
				.from("share_public")
				.select("shared_item_type, shared_item_id, shared_item_name, sender_user_id, recipient_user_id, status")
				.eq("share_id", shareId)
				.single();
...
export default function shareUpdateStatusHandler(
	...
	return Effect.gen(function* shareUpdateStatusGen($) {
		...
		const shareDetails = yield* $(getShareForRecipient(
			client,
			req.share_id,
			recipientUserId
		));
		...
		if (req.status === 'accepted') {
			...
			if (shareDetails.shared_item_type === 'playlist') {
				yield* $(
					addPlaylistSongsToUserLibrary(
						client,
						recipientUserId,
						shareDetails.shared_item_id,
					).pipe(
						Effect.catchAll((err) =>
							Effect.sync(() => {
								console.warn(
