...
export default function communityPlaylistRemove(
	...
	return Effect.gen(function* communityPlaylistRemoveGen($) {
		...
		if (!getCommunityRoleCapabilities(requesterRole.data?.role).canManageEvents) {
			return yield* $(Effect.fail(new ValidationError({ message: "Only community owners and admins can remove playlists" })));
		}
		const deleteResult = yield* $(
			Effect.tryPromise({
				...
				catch: (err) => new DatabaseError({ message: extractErrorMessage(err, "Failed to remove playlist from community") }),
			}),
		);
		if (deleteResult.error) {
			return yield* $(Effect.fail(new DatabaseError({ message: deleteResult.error.message ?? "Failed to remove playlist from community" })));
		}
		return { success: true };
	});
}
