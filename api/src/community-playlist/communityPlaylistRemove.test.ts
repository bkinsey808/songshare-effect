...
import communityPlaylistRemove from "./communityPlaylistRemove";
import makeCommunityPlaylistRemoveClient from "./communityPlaylistRemove.test-util";
...
describe("communityPlaylistRemove", () => {
	it("returns ValidationError when request body is invalid json", async () => {
		...
		const result = await Effect.runPromise(
			communityPlaylistRemove(ctx).pipe(
	...
	it("returns ValidationError when user is not owner or admin", async () => {
		...
		vi.mocked(createClient).mockReturnValue(
			makeCommunityPlaylistRemoveClient({ requesterRole: "member" }),
		);
	...
	it("returns DatabaseError when delete fails", async () => {
		vi.mocked(getVerifiedUserSession).mockReturnValue(Effect.succeed(SAMPLE_SESSION));
		vi.mocked(createClient).mockReturnValue(
			makeCommunityPlaylistRemoveClient({ deleteError: true }),
		);
	...
	it("returns success when owner removes playlist", async () => {
		vi.mocked(getVerifiedUserSession).mockReturnValue(Effect.succeed(SAMPLE_SESSION));
		vi.mocked(createClient).mockReturnValue(makeCommunityPlaylistRemoveClient());
		...
		const result = await Effect.runPromise(communityPlaylistRemove(ctx));
