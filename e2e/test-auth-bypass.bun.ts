...
async function testAuthBypass(): Promise<void> {
  ...
  try {
    ...
    const hasHomeText = pageContent.toLowerCase().includes('home') && pageContent.toLowerCase().includes('welcome');
    ...
    if (typeof bodyText === 'string' && bodyText !== '' && (bodyText.toLowerCase().includes('sign in') || bodyText.toLowerCase().includes('login'))) {
